...
// OSLO / VSDS-aligned query used in the UI
const SPARQL_QUERY_OLD = `
PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
PREFIX sosa: <http://www.w3.org/ns/sosa/>
PREFIX time: <http://www.w3.org/2006/time#>
PREFIX prov: <http://www.w3.org/ns/prov#>
PREFIX vkm: <https://data.vlaanderen.be/ns/verkeersmetingen#>
...
PREFIX sps: <http://def.isotc211.org/iso19156/2011/SamplingPoint#>
...
SELECT ?obs ?sensor ?sensorLabel ?wkt ?vehicleType ?startTime ?duration ?count
WHERE {
  ...
  ?obs vkm:geobserveerdObject ?vmp .
  ?vmp a vkm:Verkeersmeetpunt .
  OPTIONAL { ?vmp (sps:SF_SamplingPoint.shape|geosparql:hasGeometry)/geosparql:asWKT ?wkt }

  OPTIONAL { ?obs sosa:madeBySensor ?sensor . OPTIONAL { ?sensor rdfs:label ?sensorLabel } }

  OPTIONAL {
    ?obs (om:OM_Observation.phenomenonTime|sosa:phenomenonTime) ?phen .
    OPTIONAL { ?phen time:hasBeginning ?beg . ?beg time:inXSDDateTimeStamp ?startStamp }
    OPTIONAL { ?phen time:hasXSDDuration ?duration }
  }
  OPTIONAL { ?obs sosa:resultTime ?resultTime }
  OPTIONAL { ?obs prov:generatedAtTime ?generated }
  BIND (COALESCE(?startStamp, ?resultTime, ?generated) AS ?startTime)

  OPTIONAL { ?obs vsds:Verkeerstelling.tellingresultaat ?tellingRaw }
  OPTIONAL { ?obs sosa:hasSimpleResult ?simpleRaw }
  BIND (COALESCE(?tellingRaw, ?simpleRaw) AS ?count)

  OPTIONAL {
    { ?obs vsds:Verkeerstelling.geobserveerdKenmerk ?kenmerk .
      OPTIONAL { ?kenmerk vkm:voertuigType ?vehicleTypeIri }
    }
    UNION
    { ?obs sosa:observedProperty ?prop .
      OPTIONAL { ?prop vkm:voertuigType ?vehicleTypeIri }
    }
  }
  BIND (COALESCE(STR(?vehicleTypeIri), "unknown") AS ?vehicleType)
}
